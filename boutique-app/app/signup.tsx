...
export default function SignupScreen() {
  const renderHeaderLogo = () => (
    <View className="p-4 mb-4">
      <SvgXml xml={TIME_SVG} width={53} height={38} />
    </View>
  );
  ...
  const handleFinalSignup = async () => {
    ...
    try {
      ...
      const isLocalAsset = (uri: string | null) => !!uri && !/^https?:\/\//i.test(uri);

      const uploadTasks: Promise<unknown>[] = [];
      if (isLocalAsset(ownerImage)) {
        uploadTasks.push(
          (async () => {
      ...
      if (boutiqueId && isLocalAsset(logo)) {
        uploadTasks.push(
          (async () => {
            const form = new FormData();
      ...
      if (boutiqueId && isLocalAsset(frontImage)) {
        uploadTasks.push(
          (async () => {
            const form = new FormData();
      ...
      if (boutiqueId && isLocalAsset(insideImage)) {
        uploadTasks.push(
          (async () => {
            const form = new FormData();
      ...
      if (uploadTasks.length) {
        const outcomes = await Promise.allSettled(uploadTasks);
        const failed = outcomes.filter((o) => o.status === 'rejected');
        if (failed.length) {
          console.warn('Some signup images failed to upload', failed);
          Alert.alert(
            'Photos',
            'Your account is ready, but one or more photos could not be uploaded. You can add them later under Business Profile.',
          );
        }
      }
  ...
  const renderProgressBar = (current: string) => {
    const [currentStepRaw, totalStepsRaw] = current.split('/');
    const currentStep = Number(currentStepRaw);
    const totalSteps = Number(totalStepsRaw);
    const progressPercent =
      Number.isFinite(currentStep) && Number.isFinite(totalSteps) && totalSteps > 0
  ...
  const renderStepShopInfo = () => (
    <View className="flex-1">
        ...
        <View className="items-center mb-10">
            {renderHeaderLogo()}
            <Text className="mb-2" style={SIGNUP_TITLE_STYLE}>Create Store Profile</Text>
            <Text style={SIGNUP_SUBTITLE_STYLE}>Set up your boutique&apos;s branding and location details</Text>
        </View>
